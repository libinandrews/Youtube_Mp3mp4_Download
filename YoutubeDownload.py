...
class YoutubeDownloader:
    def __init__(self, root):
      ...
      #  self.root.geometry("1100x900")
        ...
        logo = os.path.join(self.current_dir, "resources",
    ...
    def load_settings(self):
        ...
        settings_file = os.path.join(
            self.current_dir, "resources", "settings.json")
        try:
    ...
    def excel_explorer(self):
        ...
        file_path = filedialog.askopenfilename(initialdir=self.current_dir, filetypes=[
        ...
        if playlist_url:
            ...
            try:
                ...
                try:
                    ...
                    print("Failed to load the link")
            df = pd.DataFrame(list(VideoData.items()),
                              columns=['Title', 'url'])
            return df
    ...
    def Loadplaylists(self):
        ...
        # pandasgui.show(self.df)
            ...
            self.dropdown_var.set(self.dropdown_values[0])
            root.update_idletasks()
        else:
    ...
    def savedata(self):
        ...
        try:
            ...
            load_df = newdf
        newdf = pd.concat([newdf, load_df], ignore_index=True)
        newdf.drop_duplicates(
            subset=['Title', 'url', 'FileType'], inplace=True)
    ...
    def check_networks(self):
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"Current time: {formatted_time}, network status is {network_available}")
                return True
                break
            except OSError:
    ...
    def download(self):
        def download_audio_from_youtube(url, output_folder):
            if self.check_networks():
                ...
                highest_bitrate_audio = audio_streams.order_by(
                    'abr').desc().first()
            try:
        ...
        df = self.df
        df = df.sort_values(
            by='FileType', ascending=True).reset_index(drop=True)
       # pandasgui.show(df)
        numb = 0
    ...
    def GUI(self):
        ...
        savefolder_frame = tk.LabelFrame(
            self.root, text="Save Folder", font=("Calibri", 12), foreground="gray")
        youtube_frame = tk.LabelFrame(
            self.root, text="youtube Data", font=("Calibri", 12), foreground="gray")
        download_frame = tk.LabelFrame(
            self.root, text="Download", font=("Calibri", 12), foreground="gray")
        disclaimer_frame = tk.LabelFrame(
            self.root, text="Disclaimer", font=("Calibri", 7), foreground="gray")
...
# TODO Label Frame Data ------------Save Folder ------------------------
        ...
        folder_button = tk.Button(
            savefolder_frame, text="Choose Destination Folder", command=self.file_explorer)
        ...
        self.folder_entry = Entry(
            savefolder_frame, textvariable=self.folder_path, width=148)
        self.folder_entry.grid(row=0, column=1, padx=0, pady=10, sticky="w")
        self.folder_entry.bind('<KeyRelease>', self.update_entry_data)

        # Create a new StringVar for displaying the content
        self.folder_content = StringVar()
        self.folder_content.set(self.folder_path.get())  # Set initial value
        chosen_folder_label = Label(
            savefolder_frame, text="Chosen Folder:", font=("CALIBRI", 10))
        chosen_folder_label.grid(row=1, column=0, padx=10, pady=0, sticky="w")
        self.folder_choosed = Label(
            savefolder_frame, textvariable=self.folder_content, font=("CALIBRI", 10))
        self.folder_choosed.grid(
            row=2, column=0, columnspan=2, padx=10, pady=0, sticky="w")
# Label 2 Youtube Data
        ...
        self.youtubelink_entry = Entry(
            youtube_frame, textvariable=self.youtubelink, width=148)
        self.youtubelink_entry.grid(
            row=0, column=1, padx=0, pady=10, sticky="w")
        self.youtubelink_entry.bind('<KeyRelease>', self.update_entry_data)
        btn_paste = tk.Button(youtube_frame, text="Paste",
        ...
        self.mp3_radio = Radiobutton(
            youtube_frame, text="Only MP3", variable=self.radio_var, value=1, command=self.mp3_button_click)
        self.mp3_radio.grid(row=6, column=0, padx=10, pady=10)
        self.video_radio = Radiobutton(
            youtube_frame, text="Only Video", variable=self.radio_var, value=2, command=self.video_button_click)
        self.video_radio.grid(row=6, column=1, padx=10, pady=10)
        self.next_button = Button(
            youtube_frame, text="Next", state=DISABLED, command=self.Loadplaylists)
        self.next_button.grid(row=7, column=0, padx=10, pady=10)

        self.dropdown_var = StringVar()
        self.dropdown = ttk.Combobox(
            youtube_frame, textvariable=self.dropdown_var, values=self.dropdown_values, width=125)
        self.dropdown.grid(row=8, column=0, columnspan=2,
                           padx=10, pady=10, sticky='w')
        self.delete_dropdown_button = Button(
            youtube_frame, text="Delete Dropdown Item", command=self.delete_dropdown_item)
        self.delete_dropdown_button.grid(
            row=8, column=1, padx=10, pady=10, sticky="E")

        self.savebttm = Button(
            youtube_frame, text="Save Data", command=self.savedata)
        self.savebttm.grid(row=9, column=0, padx=10, pady=10, sticky="E")

# Label Frame 3 Download
        self.loadbttn = Button(
            download_frame, text="Load Data for Download", command=self.loaddata)
        self.loadbttn.grid(row=0, column=0, padx=10, pady=10, sticky="E")
        self.excelbtn = Button(
            download_frame, text="Open in Excel", command=self.openinexcel)
        self.excelbtn.grid(row=0, column=1, padx=10, pady=10, sticky="E")
        self.editexcelbtn = Button(
            download_frame, text="Load from Excel", command=self.excel_explorer)
        self.editexcelbtn.grid(row=0, column=2, padx=10, pady=10, sticky="E")
        self.cleardatabtn = Button(
            download_frame, text="clear data", command=self.cleardata)
        self.cleardatabtn.grid(row=0, column=3, padx=10, pady=10, sticky="E")

        self.Downlaodbtn = Button(
            download_frame, text="Download", command=self.download)
        self.Downlaodbtn.grid(row=1, column=0, columnspan=3,
                              padx=10, pady=10, sticky="nsew")
        self.progressbar = ttk.Progressbar(
            download_frame, orient='horizontal', mode='determinate', length=1000)
        self.progressbar.grid(column=0, row=4, columnspan=3,
...
# Label 3 Frame Data -----------disclaimer_frame ------------------------
        ...
        label = tk.Label(disclaimer_frame, text=self.label_text,
                         wraplength=self.width-25, fg="gray", font=("Calibri", 7))
